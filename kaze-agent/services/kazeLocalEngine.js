...
const MODEL = process.env.KAZE_EDGE_MODEL || 'gemini-2.5-flash';
...
function discoverKazeEdgeRepo() {
  return firstExisting([
    process.env.KAZE_EDGE_REPO,
    process.env.KAZE_EDGE_DIR,
    path.join(os.homedir(), 'Videos', 'kazeEdge-agent-main', 'kazeEdge-agent-main'),
    path.join(os.homedir(), 'Videos', 'projecto kazeEdge', 'kazeEdge-agent-main', 'kazeEdge-agent-main'),
  ]);
}

function discoverKazeEdgePython(repoPath) {
  return firstExisting([
    process.env.KAZE_EDGE_PYTHON,
    repoPath && path.join(repoPath, 'venv', 'Scripts', 'python.exe'),
  ]);
}

function buildKazeEdgeEnv(repoPath) {
  const geminiKey = process.env.GEMINI_API_KEY || process.env.VITE_GEMINI_API_KEY || process.env.GOOGLE_API_KEY || '';
  const kazeEdgeHome = process.env.KAZE_EDGE_HOME || path.join(process.cwd(), 'kaze-agent', '.kazeEdge-kaze');

  fs.mkdirSync(kazeEdgeHome, { recursive: true });

  return {
    ...process.env,
    GEMINI_API_KEY: geminiKey,
    GOOGLE_API_KEY: geminiKey,
    KAZE_EDGE_HOME: kazeEdgeHome,
    PYTHONUTF8: '1',
    PYTHONIOENCODING: 'utf-8',
    KAZE_EDGE_QUIET: '1',
    KAZE_EDGE_REPO: repoPath,
  };
}
...
function listSkills() {
  const repoPath = discoverKazeEdgeRepo();
  if (!repoPath) return [];
...
function parseKazeEdgeOutput(stdout) {
...
function executeKazeEdgeTask({ task, skill, dryRun = false, context = {} }) {
  const repoPath = discoverKazeEdgeRepo();
  const pythonPath = discoverKazeEdgePython(repoPath);
  const runnerPath = path.join(__dirname, 'kazeEdgeRunner.py');

  if (!repoPath) {
    throw new Error('KazeEdge repo não encontrado. Define KAZE_EDGE_REPO ou KAZE_EDGE_DIR.');
  }

  if (!pythonPath) {
    throw new Error('Python do KazeEdge não encontrado. Define KAZE_EDGE_PYTHON.');
  }

  if (!fs.existsSync(runnerPath)) {
    throw new Error('Wrapper do KazeEdge não encontrado.');
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code) => {
      const parsed = parseKazeEdgeOutput(stdout);
      ...
      lastError = stderr.trim() || parsed.response || `KazeEdge terminou com código ${code}`;
...
function queueExecution(payload) {
  const run = activeExecution
    .catch(() => null)
    .then(() => executeKazeEdgeTask(payload));
...
server.listen(PORT, HOST, () => {
  console.log(`[KazeEdge Bridge] Activo em http://${HOST}:${PORT}`);
});
