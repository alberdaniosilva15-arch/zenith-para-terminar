-- Atualizar validate_tracking_token para suportar ride_id, public_token e retornar dados GPS completos
...
DECLARE
  v_ride_id UUID;
BEGIN
  -- 1. Verificar se é um public_token em ride_tracking_shares
  SELECT rts.ride_id INTO v_ride_id
  FROM public.ride_tracking_shares rts
  WHERE rts.public_token = p_token AND rts.expires_at > NOW()
  LIMIT 1;

  -- 2. Se não for, verificar se é o public_token ou o próprio ID da corrida em rides
  IF v_ride_id IS NULL THEN
    SELECT r.id INTO v_ride_id
    FROM public.rides r
    WHERE r.id = p_token OR r.public_token = p_token
    LIMIT 1;
  END IF;
